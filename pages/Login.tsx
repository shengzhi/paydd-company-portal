...
const Login: React.FC = () => {
  ...
  const { login } = useAuth(); // We might need to bypass this if we implement custom logic
  ...
  const doLogin = async (captchaResult: any) => {
    try {
      console.log('Captcha Result:', captchaResult);
      // 1. Verify Ticket to get UUID
      const uuid = await loginService.verifyTicket({
        sceneId: 'test_scene_id', // Replace with actual SceneId
        certifyId: captchaResult.certifyId,
        data: JSON.stringify(captchaResult) // Pass full result as data if needed
      });

      // 2. Sign In
      const res = await loginService.signIn({
        account: activeTab === 'email' ? email : mobile,
        password: activeTab === 'email' ? password : undefined,
        code: activeTab === 'mobile' ? verificationCode : undefined,
        loginType: activeTab,
        phoneDistrict: activeTab === 'mobile' ? countryCode : undefined,
        uuid: uuid
      });

      // 3. Handle Success
      localStorage.setItem('token', res.accessToken);
      // Update Auth Context (hacky direct update for now, ideally use setSession)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Initialize/Trigger Aliyun Captcha
    if (window.initAliyunCaptcha) {
      window.initAliyunCaptcha({
        SceneId: 'test_scene_id', // Replace with actual SceneId
        prefix: '198', // Example prefix
        mode: 'popup', // 'popup' or 'embed'
        element: '#captcha-element', // If embed
        button: '#submit-button', // If popup, binding button
        captchaVerifyCallback: (captchaResult: any) => {
          doLogin(captchaResult);
        },
        onBizResultCallback: (bizResult: any) => {
          console.log('Biz Result:', bizResult);
        },
        getInstance: (instance: any) => {
          instance.verify(); // Trigger verification immediately
        },
        slideStyle: {
          width: 360,
          height: 40,
      ...
      setIsSubmitting(false);
      alert("Captcha SDK not loaded. Please refresh.");
    }
  };
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="flex p-1 bg-slate-100 rounded-xl">
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'email' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 flex items-center justify-center gap-2 ${activeTab === 'mobile' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
            >
