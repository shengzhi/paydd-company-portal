...
import { 
  ...
  Building, Phone, FileText, Users, ShieldCheck, ArrowRight, Save
} from 'lucide-react';
                   ...
                   <div className="overflow-hidden border border-slate-200 rounded-lg">
                      <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-200">
                          {MOCK_SHAREHOLDERS.map((item) => (
                            <tr key={item.id} className="hover:bg-slate-50">
                              <td className="px-4 py-3 text-sm text-slate-900 font-medium">{language === 'zh-CN' || language === 'zh-TW' ? '个人' : item.entityType}</td>
                              <td className="px-4 py-3 text-sm text-slate-700">{language === 'zh-CN' || language === 'zh-TW' ? '布施' : item.name}</td>
                              <td className="px-4 py-3 text-sm text-slate-600 flex items-center gap-2">
                                <img src="https://flagcdn.com/w20/us.png" alt="US" className="w-5 h-3.5 object-cover rounded-[1px]" />
                                {language === 'zh-CN' || language === 'zh-TW' ? '美国' : 'USA'}
                              </td>
